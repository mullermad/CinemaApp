  ...
  <div class="flex min-h-screen bg-gray-900">
    ...
    <main class="flex-1 p-4 lg:ml-64">
      <div class="p-4 rounded-lg border-gray-700">
        ...
        <!-- Add New Star Button -->
        <button @click="toggleForm" data-cy="add-star"
        ...
        <div v-if="showForm" class="mb-4 p-4 bg-gray-800 border border-gray-700 rounded-lg">
          ...
          <form @submit.prevent="handleSubmit">
            <div class="mb-4">
              <label for="starName" class="block text-gray-300 mb-2">Name</label>
              <input id="starName" v-model="newStarName" data-cy="newStarName" name="newStarName" type="text"
                class="w-full p-2 border border-gray-600 rounded-lg" required placeholder="Enter star's name" />
            </div>
            <div>
              ...
              <input type="file" @change="handleFeaturedImagesChange" multiple
                class="text-white w-full p-3 border rounded-lg border-gray-700" />
            </div>

            <button data-cy="add-star-button" type="submit"
            ...
            <tbody>
              <tr v-for="star in paginateStars" :key="star.id">
                ...
                <td class="p-3 text-right">
                  <button data-cy="delete-star" class="text-red-600 hover:underline text-sm"
        ...
        <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
          <span
            class="flex items-center px-3 h-8 leading-tight border rounded-s-lg  bg-gray-800 border-gray-700 text-gray-400 hover:bg-gray-700 hover:text-white">
          ...
          <div class="w-full max-w-md bg-white shadow-lg rounded-md p-6 relative">
            ...
            <div class="my-8 text-center">
              ...
              <div class="flex justify-center mt-4">
                <button @click="confirmDelete" data-cy="confirm-delete-star"
                  class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600">
                  Confirm
                </button>
                <button @click="showModal = false" data-cy="cancel-delete-star"
...
async function handleFeaturedImagesChange(event) {
  ...
  if (files.length > 0) {
    try {
      ...
      stars.value.poster_url = imageUrls;
...
const handleSubmit = async () => {
  const variables = {
    object: {
      ...
      poster_url: stars.value.poster_url || [],  // Send poster_urls as an array
