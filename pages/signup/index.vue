    ...
    <div class="w-full max-w-md bg-gray-900 p-8 rounded-lg shadow-lg">
      <form @submit.prevent="onSubmit" class="space-y-6">
        ...
        <div>
          ...
          <ErrorMessage name="password" class="create-stronger-password text-red-500 text-sm italic" />
        </div>
        <div>
          ...
          <div class="relative">
            <Field name="confirmPassword" :type="showConfirmPassword ? 'text' : 'password'"
              placeholder="Confirm your password" class="w-full p-3 border border-gray-700 rounded-lg shadow-sm" />
            <button type="button" @click="showConfirmPassword = !showConfirmPassword"
              class="absolute inset-y-0 right-0 pr-3 flex items-center">
              <svg v-if="showConfirmPassword" class="w-8 h-8 text-gray-400" fill="none" stroke="currentColor"
                viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M12 19.5c5.5 0 10-7.5 10-7.5s-4.5-7.5-10-7.5-10 7.5-10 7.5 4.5 7.5 10 7.5zM12 14a2 2 0 1 0 0-4 2 2 0 0 0 0 4z" />
              </svg>
          ...
          <ErrorMessage name="confirmPassword" class="confirm-password-error-message text-red-500 text-sm italic" />
        </div>
