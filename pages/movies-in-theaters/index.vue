<template>
  <section class="bg-gray-900 relative w-full min-h-screen bg-cover bg-center pt-16">
    <!-- Loading State -->
    <div v-if="loading"
      class="absolute inset-0 flex items-center justify-center bg-gray-900 bg-opacity-60 text-white text-xl font-bold">
      Loading...
    </div>
    ...
    <div v-if="!loading && !error" class="relative z-10 max-w-screen-xl mx-auto px-6">
      ...
      <div
        class="flex flex-col lg:flex-row lg:items-center lg:justify-between mt-4 space-y-4 lg:space-y-0 lg:space-x-4">
        <!-- Genre Filter -->
        <div class="flex flex-wrap gap-2">
          <button v-for="genre in uniqueGenres" :key="genre" @click="filterByGenre(genre)" :class="{
            'bg-blue-600': activeGenre === genre,
            'bg-gray-900': activeGenre !== genre
          }"
            class="genere-filter px-4 py-2 rounded-full text-white font-medium hover:bg-blue-700 transition duration-300">
            {{ genre }}
          </button>
        </div>
        ...
        <div class="w-full lg:max-w-xs">
          <select v-model="activeDirector" @change="searchMovies"
      ...
      <div v-if="filteredMovies.length === 0"
        class="no-movie-found text-center my-8 text-xl font-semibold text-gray-300">
...
const GET_MOVIES_QUERY = gql`
  query GetMovies( $title: String, $genre: String, $director: String) {
    movie(
