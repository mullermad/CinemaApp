<template>
  <section class="bg-gray-900 relative w-full min-h-screen bg-cover bg-center pt-16">
    <div v-if="!loading && !error" class="relative z-10">
      <h2 class="text-2xl font-bold text-white drop-shadow-lg p-4">
        MOVIES IN THEATERS
      </h2>
      ...
      <div v-if="filteredMovies.length > 0">
        ...
        <div class="flex justify-center mt-4">
          <span v-for="page in totalPages" :key="page" @click="goToPage(page)" :class="{
            'w-3 h-3 mx-2 rounded-full cursor-pointer': true,
      ...
      <div class="absolute top-4 right-4 z-30 flex flex-row items-center space-x-4 pt-5 pr-7 pb-2">
        <nuxt-link to="/movies-in-theaters" class="text-blue-400 hover:underline font-semibold">See All
          Movies</nuxt-link>
      </div>
    </div>
  </section>
